# IPTV Playlist – Hindi & Punjabi Channels (M3U)

This GitHub repository provides a **free IPTV M3U playlist** containing **300+ Hindi and Punjabi TV channels**.  
The playlist is created for users who want a **simple, lightweight, and ad-free IPTV source** that works across multiple IPTV players and devices.

This repository is intended for **educational and personal use** and uses only **publicly available free-to-air streams**.


## 📺 Playlist URL (Copy & Paste)

https://raw.githubusercontent.com/deep2772/Hindi_Punjabi-iptv-playlist/refs/heads/main/Hindi_Punjabi_Merged.m3u


---

## 📂 Playlist Information

- **File type:** M3U  
- **Content:** Free IPTV streams  
- **Languages:** Hindi, Punjabi  
- **Region:** India  
- **Duplicates:** Removed by channel name  
- **Hosting:** GitHub (RAW access)

---

## ▶️ Compatible IPTV Players

This IPTV playlist works with most popular players, including:

- TiviMate (Android TV)
- IPTV Smarters (Android / iOS)
- OTTPlayer (WebOS / Web)
- VLC Media Player (Windows / macOS / Linux)
- MyIPTV Player (Windows)

---

## 🛠 How to Use This Playlist

1. Open your IPTV application
2. Choose **Add Playlist**
3. Select **M3U URL**
4. Paste the playlist URL from above
5. Update or refresh the playlist

For EPG:
- Add the XML URL in **EPG / TV Guide settings**

---

## 🔎 Keywords (For Search Engines)

IPTV playlist  
GitHub IPTV  
Hindi IPTV M3U  
Punjabi IPTV playlist  
Free IPTV channels  
Indian IPTV playlist  
IPTV M3U GitHub  

---

## ⚠️ Disclaimer

- This repository **does not host any video streams**
- All links point to **publicly available sources**
- Channel availability may change without notice
- No copyright infringement is intended

---

## ⭐ Support the Project

If you find this playlist useful:
- ⭐ Star the repository
- 🔄 Share it with others
- 🍴 Fork and customize it

---

## 📌 Repository Topics

iptv, iptv-playlist, m3u, hindi, punjabi, free-iptv


## Keywords
IPTV playlist
Hindi IPTV
Punjabi IPTV
Free IPTV channels
